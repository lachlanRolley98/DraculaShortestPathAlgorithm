...
////////////////////////////////////////////////////////////////////////
/*
 * Given a radius (i.e. how many rounds) get all possible moves that a player can make
 * Radius effectively specifies how large the breadth first search should be
 * Function is relatively expensive as it gets all move combinations that a player can make with 'n' radius size
*/
PlaceId *HvplayermoveRadius(HunterView hv, Player player,  PlaceId location, int radius, int *numReturnedLocations);

/*
 * Get the location history of a given player 
 * Function is wrapper for GvGetLocationHistory
*/
PlaceId *HvlocationHistory(HunterView hv, Player player, int *numReturnedLocs);

/*
 * Returns a true of false if a given location, src, is connected to another location.
 * Function calls GvGetReachable as uses a for loop to test if src == location
*/
bool isConnected(HunterView hv, PlaceId src, PlaceId location); // is a given location adjacent to the source
