...
PlaceId DraculaMoveID(DraculaView dv, int round);								  // Gets dracula's move, either returns the location he was at or double back, hide
...
PlaceId doubleBackLocation(DraculaView dv, int doubleBackSize);					  // Returns the location a double back is referring to
bool AdjacentToEndOfTrail(DraculaView dv, PlaceId nextLocation);				  // checks if the given location is adjacent to the end of the trail
PlaceId *DvWhereCanIGoFromLoc(DraculaView dv, int *numReturnedLocs, PlaceId src); // does exactly what DvWhereCanIGo does but instead takes in a src instead of the current location
...
PlaceId *DvWhereCanIGo(DraculaView dv, int *numReturnedLocs)
{
	...
	int placeCount = 0;
	for (int i = 0; i < *numReturnedLocs; i++) // loops through each reachable location
	{
		if (!hasBeenVisited(dv, reachable[i])) // checks that the location is not in the trail
		{
...
int *mapWeightings(DraculaView dv)
{
	...
	for (int i = 0; i < numReturnedLocs; i++)
	{ // loops through dracula's valid moves and outputs all the locations he can visit
		...
		bool inLocations = false; // checking if that location has already been added to the array
		for (int i = 0; i < numLocs; i++)
		{
			if (moveLocations[i] == location)
				inLocations = true;
		}
		if (!inLocations)
		{
	...
	for (int i = 0; i < numLocs; i++)
	{						  // loop through each location that dracula can move to
		...
		int weight = 0;
		if (minDist == 0)
			weight = LOCATION_IS_REACHABLE_BY_HUNTER;
		else if (!placeIsSea(moveLocations[i]))
			weight = round(exp((minDist - 7.0) / (-1.7)));
		...
		{
			weightedLocs[moveLocations[i]] += numLocations;
		} // reward locations for having more connections
	}
	return weightedLocs;
}

// returns the number of players near Castle dracula, within a given round radius
...
// returns the shortest path to a location, exluding seas. Used when dracula wants to go to Castle dracula but he is on critical health
PlaceId *DvGetSafestPathTo(DraculaView dv, Player player, PlaceId dest, int *pathLength)
{
	...
	for (int i = 0; i < NUM_REAL_PLACES; i++)
		visited[i] = -1; // initialise visited array to all -1's
...
bool hideMoveValid(DraculaView dv, char *pastPlays, Round round, int size)
{
	if (placeIsSea((DvGetPlayerLocation(dv, PLAYER_DRACULA))))
		return false;						 // Dracula cannot hide while at sea
...
bool hasBeenVisited(DraculaView dv, PlaceId location)
{
	...
	if (location == DvGetPlayerLocation(dv, PLAYER_DRACULA))
		return true; //! Quick fix: Problem is that without this dracula will try to move back to his current location
	...
	int specialMoves = 0;
	for (int i = 0; i < numReturnedLocs; i++)
	{													 // loop through and add locations that have been visited through hiding or double backing
	...
	for (int i = 0; i < specialMoves; i++)
	{ // loop through locations in his trail visited by double back or hide
		bool ValidMoveFound = false;
		for (int j = 0; j < numReturnedLocs; j++)
		{ // loop through locations in the trail
			if (trail[j] == specialMoveLocs[i] && placeIsReal(DraculaMoveID(dv, dv->round - j)))
			{ // if the location has been visited by a double back or hide AND by a normal move to that location
				ValidMoveFound = true;
			}
		}
		if (!ValidMoveFound)
		{ // if not found, then he can revisit that location this turn
	...
	for (int i = 0; i < numReturnedLocs; i++)
	{ // loop through trail and add locations that are not in the blacklisted array
	...
	free(trail), free(blacklist), free(specialMoveLocs);

	for (int i = 0; i < trailSize; i++)
	{
		if (location == newTrail[i]) // check if the location is in the trail
		{
			free(newTrail);
			return true;
		}
...
bool isAdjacent(DraculaView dv, int doubleBackSize, PlaceId location)
{
	...
	{ // search through all connected cities
...
// checks if the given location is adjacent to the end of the trail, and thus he can move back into his trail after next turn
// used to check if the location will not cause a teleport move if he moves there
bool AdjacentToEndOfTrail(DraculaView dv, PlaceId nextLocation)
{
	...
	for (int i = 0; i < numConns; i++)
	{ // check if the given location is in the connections array
...
// does exactly what DvWhereCanIGo does but instead takes in a src, so that it can be used to see reachable location from a hypothetical locations
PlaceId *DvWhereCanIGoFromLoc(DraculaView dv, int *numReturnedLocs, PlaceId src)
{
	...
	for (int i = 0; i < *numReturnedLocs; i++) // loops through each reachable location
	{
		if (!hasBeenVisited(dv, reachable[i]))
		{
			validPlaces[placeCount] = reachable[i]; // checks that the location is not in the trail
			placeCount++;
		}
	}
	*numReturnedLocs = placeCount;
