...
// wrapper for the moveRadius function from GameView.c, allows dracula to see what locations are more accessible to hunters
PlaceId *playermoveRadius(DraculaView dv, Player player, int radius, int *numReturnedLocations);

// returns locations that can't be accessed by all the hunters within a certain move radius, used to decide dracula's starting location,
PlaceId *unreachables(DraculaView dv, int moves, int *numReturnedLocations);

// returns the number of connections from a certain location for dracula, ie. excludes seas
int locationConnections(DraculaView dv, PlaceId src);

// wrapper for the same function in GameView.c, see the .h for it there
...
// returns the locations in dracula's trail
...
typedef enum weightings
{                                         // all weightings should be less than or equal to 40 (points can become -ve)
    LOCATION_START_WEIGHTING = 40,        // each location starts with 40 points
    HUNTER_AT_LOCATION = 10,              // 10 points are lost if a hunter is at a given city (No encounter can occur if Dracula moves to a location occupied by a hunter,
                                          // however other hunters can still move to that location so it isn't entirely safe)
    LOCATION_IS_SEA = 2,                  // 2 points lost if location is at sea (Utility function possibility: Cost = -0.45*HP + 20 i.e. cost increase as blood points decreases)
                                          // Also note that the funtion increases slowly (to account for the fact that 2 blood points loss isn't that bad initially) and that Cost(40) = 2.
    LOCATION_IS_CASTLE_DRACULA = 15,      // 10 points lost (ideally dracula doesn't want to go to CD)
                                          // if location is castle dracula (Castle Dracula utility function = 10 * (current blood points/40)
                                          // meaning that the lower the blood points the less is subtracted from the weighting).
    LOCATION_IS_REACHABLE_BY_HUNTER = 34, // If a location is directly reachable by a hunter location loses 20 points
    LOCATION_IS_DISTANTLY_REACHABLE = 10, // If a location is reachable by a hunter in there combination of moves 2 rounds ahead penalise location
} Weightings;
